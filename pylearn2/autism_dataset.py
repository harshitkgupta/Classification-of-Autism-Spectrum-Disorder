...
class AutismDataset( DenseDesignMatrix ):
    ...
    def __init__(self, 
		...
		super(AutismDataset, self).__init__(X=X, y=y)
    ...
    def _load_data(self, path, expect_labels):
		...
		data = np.loadtxt( path, delimiter = ',', dtype = 'float64' )
		...
		if expect_labels:
			...
			one_hot = np.zeros((y.shape[0], self.no_classes ),dtype='float64')
